...
describe "ActiveRecord American Gladiator" do
  ...
  context "Powerball" do
    it "returns all items containing Powerball" do
      ...
      # Changeable Start
      items = Item.select(:name).where('name like "Powerball%"')
      # Changeable End
  ...
  context "Hang Tough" do
    it "returns orders for 3 users in 2 queries (aka: Remove the N+1 query)" do
      ...
      # Changeable Start
      users = User.eager_load(:orders).limit(3)
      # Changeable End
  ...
  context "The Maze" do
    it "returns all users that have placed an order" do
      ...
      # Changeable Start
      active_users = User.joins(:orders).where('orders.user_id = users.id').distinct
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      orders = Order.joins(:items).where('name like "wrestling_ring" or name like "football"')
  ...
  context "The Eliminator" do
    it "returns all orders placed 2 weeks ago" do
      ...
      # Changeable Start
      orders = Order.where('created_at <= ?', 14.days.ago)
      # orders = Order.all.select do |order|
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      most_popular_items = Item.select('items.*, COUNT(order_items.item_id) as "item_count"').joins(:orders).group('order_items.item_id').order('"item_count" DESC').limit(2)
